...
from .data_loader import MitbihTrainDataLaoder
...
class SyntheticMitbihDataLoader(Dataset):
    def __init__(self, n_samples=20000, seq_len=187, reshape=False, model_path='./mitbih_checkpoint'):
        gen_net = TCGANModel(seq_len=seq_len, channels=1, num_classes=5, latent_dim=100, data_embed_dim=10,
        ...
        self.labels = np.concatenate((np.array([0] * n_samples), np.array([1] * n_samples), np.array([2] * n_samples),
                                      np.array([3] * n_samples), np.array([4] * n_samples)))

        print(f'data shape is {self.data.shape}')
        print(f'labels shape is {self.labels.shape}')
        print(
            f'The dataset including {n_samples} class 0, {n_samples} class 1, {n_samples} class 2, {n_samples} class '
            f'3, {n_samples} class 4')
...
class MixedSyntheticMitbihDataLoader(Dataset):
    def __init__(self, real_samples=200, syn_samples=800):
        syn_ecg = SyntheticMitbihDataLoader(n_samples=syn_samples, reshape=True)
        real_ecg = MitbihTrainDataLaoder(n_samples=real_samples, oneD=True)
