...
def calculate_gradient_penalty(y, x, args):
...
def calculate_gradient_penalty(out_src, x_hat, args):
    # ... (your calculate_gradient_penalty function implementation) ...
    pass


def train(gen_net: nn.Module, dis_net: nn.Module, train_loader, epoch, writer_dict, args,
          gen_optimizer=None, dis_optimizer=None, gen_avg_param=None,
          schedulers=None):
    ...
    for iter_idx, (real_imgs, real_img_labels) in enumerate(tqdm(train_loader)):
        ...
        real_img_labels = real_img_labels.type(torch.LongTensor).to(device)

        noise = torch.FloatTensor(np.random.normal(0, 1, (real_imgs.shape[0], args.latent_dim))).to(device)
        fake_img_labels = torch.randint(0, 5, (real_imgs.shape[0],)).to(device)
        ...
        d_loss_gp = calculate_gradient_penalty(out_src, x_hat, args)
...
def train2(gen_net: nn.Module, dis_net: nn.Module, train_loader, epoch, writer_dict, args,
          gen_optimizer=None, dis_optimizer=None, gen_avg_param=None,
          schedulers=None):
    writer = writer_dict['writer']
    gen_step = 0
    ...
    for iter_idx, (real_imgs, real_img_labels) in enumerate(tqdm(train_loader)):
        ...
        assert fake_imgs.size() == real_imgs.size(), f"fake_imgs.size(): {fake_imgs.size()} real_imgs.size(): " \
                                                     f"{real_imgs.size()}"
